...
def main():
    # 引数があればそれを使用、なければ r26a_urdf_description を探す
    if len(sys.argv) > 1:
        target_dir = sys.argv[1]
    else:
        # 実行場所に合わせて r26a_urdf_description へのパスを自動設定
        if os.path.exists("src/r26a_urdf_description"):
            target_dir = "src/r26a_urdf_description"
        elif os.path.exists("r26a_urdf_description"):
            target_dir = "r26a_urdf_description"
        else:
            target_dir = "." # 見つからなければカレントディレクトリ
