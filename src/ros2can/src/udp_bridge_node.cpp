#include <udp_bridge_node.hpp>


UdpBridgeNode::UdpBridgeNode() : Node("udp_bridge_node"), sockfd_(-1), is_running_(true) {
...
void UdpBridgeNode::tx_callback(const ros2can::msg::UdpCanFrame::SharedPtr msg) {
    if (sockfd_ < 0) return;
...
void UdpBridgeNode::rx_thread_func() {
    UdpPacket packet;
